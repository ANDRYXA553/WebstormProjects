...
for (let user of users) {
    ...
    divAdress.style.backgroundColor = 'grey'
    divAdress.style.margin = '15px 0 15px 0'

    for (let userProp in user) {
        let divProp = document.createElement('div')

        divProp.innerHTML = `${userProp}:${user[userProp]}`
        if (userProp === 'address') {
            divProp.innerHTML = `${userProp}:`
        }
        divUser.appendChild(divProp)
    }
    for (let adressProp in user.address) {
        let divAdressProp = document.createElement('div')
        divAdressProp.innerHTML = `${adressProp}:${user.address[adressProp]}`
        divAdress.appendChild(divAdressProp)
        divUser.appendChild(divAdress)
    }
